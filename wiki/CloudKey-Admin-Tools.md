...
stack" header, no autossh section, and no Tailscale section — not three
headers full of "not installed" lines. Within a section that IS
showing (say, only 2 of the 4 media apps are installed), the
not-yet-installed ones still get their own line, since a partially-built
phase is worth calling out. If literally nothing optional has been
built yet, the dashboard prints one line saying so instead of a blank
screen.
